...
module RAPID2DMakieExt
    ...
    function RAPID2D.plot_snaps0D(snap0D; kwargs...)
        ...
        times_ms = [s.time_s * 1.0e3 for s in snap0D]
    ...
    function RAPID2D.plot_snaps2D(
        ...
        ax = Axis(
            fig[1, 1], xlabel = "R (m)", ylabel = "Z (m)", aspect = DataAspect(),
    ...
    """
    function RAPID2D.animate_snaps2D(
            snaps2D, R1D, Z1D;
        ...
        title_text = text!(
            ax.scene, 0.5, 1.05, text = @lift("$(field) at t = $(round(snaps2D[$frame_idx].time_s * 1.0e3, digits = 2)) ms"),
