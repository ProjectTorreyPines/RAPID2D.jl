@testsnippet EigSourceFixtures begin
    using RAPID2D: ExpRB, ForwardEuler, KnownRate, LinearResponse, EigenvalueSource,
    ...
    function eig_RAPID(;
            ...
            coulomb = false, source = KnownRate
        )
...
@testitem "eigenvalue source: the default asks for nothing it cannot read off" setup = [EigSourceFixtures] begin
    using RAPID2D: KnownRate, LinearResponse, SimulationFlags

    # `KnownRate` uses the rate the model already states as `ν·y`; `LinearResponse`
    # adds `∂ν/∂y` from the tables. Both produce a rate — the axis is whether a
    # derivative was taken, not whether the answer is one.
    @test SimulationFlags{Float64}().exprb_eigenvalue === KnownRate
    @test KnownRate !== LinearResponse
end

@testitem "eigenvalue source: growth cannot tell the two apart" setup = [EigSourceFixtures] begin
    using RAPID2D: ExpRB, KnownRate, LinearResponse
    ...
    for source in (KnownRate, LinearResponse)
        RP = eig_RAPID(; source = source)
        RP.flags.scheme.growth = ExpRB
        RP.dt = 2.0e-5
        update_RRCs!(RP)
        solve_electron_continuity_equation!(RP)
        source === KnownRate && (global ref_ne = copy(RP.plasma.ne))
        source === KnownRate && (global ref_z = copy(RP.plasma.exprb.z_growth))
        source === KnownRate && (global ref_iz = copy(RP.reactions.counts.iz))
...
@testitem "eigenvalue source: KnownRate cannot produce a growth branch on Tₑ" setup = [EigSourceFixtures] begin
    using RAPID2D: KnownRate, LinearResponse
    ...
    for Te in (0.05, 0.3, 1.0, 5.0), u in (-1.0e5, -4.5e5)
        known = eig_Te_at(eig_RAPID(; Te_eV = Te, u_para = u, source = KnownRate))
    ...
    kr = eig_Te_at(eig_RAPID(; Te_eV = 0.3, u_para = -4.5e5, source = KnownRate))
    inw = eig_RAPID().G.nodes.in_wall_nids
    @test any(>(0), lr[inw])
    @test all(<=(0), kr[inw])
end

@testitem "eigenvalue source: KnownRate is the 𝔅 of the terms carrying an explicit Tₑ" setup = [EigSourceFixtures] begin
    using RAPID2D: KnownRate
    ...
    RP = eig_RAPID(; Te_eV = 3.0, coulomb = true, source = KnownRate)
...
@testitem "eigenvalue source: KnownRate never asks update_RRCs! for a derivative" setup = [EigSourceFixtures] begin
    using RAPID2D: ExpRB, KnownRate, LinearResponse

    # The derivative surfaces are the expensive half of the branch. Under
    # KnownRate nothing reads them, so nothing should pay to build them.
    kr = eig_RAPID(; source = KnownRate)
...
@testitem "eigenvalue source: only LinearResponse needs a differentiable rate" setup = [EigSourceFixtures] begin
    using RAPID2D: ExpRB, KnownRate, LinearResponse, SimulationFlags, validate_scheme_flags

    # The legacy rate paths are refused for LinearResponse because there is no
    # dK/dĒ to take. KnownRate takes no derivative at all, so the same pairing is
...
@testitem "eigenvalue source: LinearResponse is refused where no one implements it" setup = [EigSourceFixtures] begin
    using RAPID2D: ExpRB, LinearResponse, KnownRate, SimulationFlags, validate_scheme_flags
    ...
    # would silently get KnownRate, which is the defect class this branch spent
    # its review fixing.
    lr = SimulationFlags{Float64}()
    ...
    kr.Ampere = false
    @test kr.exprb_eigenvalue === KnownRate
    @test validate_scheme_flags(kr) === kr
end
