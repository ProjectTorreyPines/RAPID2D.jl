@testitem "Circuit Time Evolution" setup = [CircuitHelpers] begin
    ...
    @testset "Single RL Circuit Step Response" begin
        ...
        dt = 1.0e-6
        t_start = 0.0
        t_end = 5.0e-3  # 5 time constants (L/R = 1ms)
        ...
        for θimp in [0.0, 0.5, 1.0]
            ...
            if θimp in [0.0, 1.0]
                @test isapprox(I_numerical, I_analytic, rtol = 1.0e-3)
                # For θimp = 0.0 or 1.0, we expect a less accurate solution
                @test !isapprox(I_numerical, I_analytic, rtol = 1.0e-5)
            elseif θimp == 0.5
    ...
    @testset "Two Coupled Coils" begin
        ...
        L1 = 1.0e-3    # Self inductance coil 1 [H]
    ...
    @testset "Two Coupled Coils - Analytical Comparison" begin
        ...
        dt = 1.0e-6
        t_end = 1.0e-3  # Longer simulation to reach steady state
        csys.Δt = dt
        ...
        @test isapprox(I1_numerical, I1_analytical, rtol = 1.0e-4)
        @test isapprox(I2_numerical, I2_analytical, rtol = 1.0e-4)
    end

    @testset "Energy Conservation in coupled LR System" begin
        # Verify energy conservation in coupled coil system
        L1, L2, M = 1.0e-3, 1.5e-3, 0.5e-3
        R1, R2 = 1.0, 1.0
        V1, V2 = 5.0, 0.0
        ...
        @test isapprox(integrated_energy_change, magnetic_energy[end] - magnetic_energy[1], rtol = 1.0e-2)
...
@testitem "RAPID coils evolution without plasma - coupled coils vs analytical" setup = [CircuitHelpers] begin
    ...
    @test isapprox(I1_numerical, I1_analytical, rtol = 1.0e-2)
    @test isapprox(I2_numerical, I2_analytical, rtol = 1.0e-2)
