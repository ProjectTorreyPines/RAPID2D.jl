@testitem "exprb_bern: the identity B(-z)/B(z) = exp(z)" begin
    ...
    function worst_identity_error()
        ...
        for z in range(-200.0, 200.0, 4001)
            iszero(z) && continue
            err = Float64(abs(big(exprb_bern(-z) / exprb_bern(z)) - exp(big(z))) / exp(big(z)))
...
@testitem "exprb_bern: z/expm1(z) is accurate at zero — no series branch needed" begin
    ...
    @test exprb_bern(-0.0) === 1.0
    @test exprb_bern(1.0e-300) == 1.0
end

@testitem "exprb_bern: strictly positive, so the diagonal can never change sign" begin
    using RAPID2D: exprb_bern
    ...
    @test all(<(0), diff(exprb_bern.(range(-50.0, 30.0, 500))))

    # The two limits the design note reads physically.
    @test exprb_bern(-50.0) ≈ 50.0 rtol = 1.0e-12   # z → −∞: B → |z|, cell fully relaxed
    @test exprb_bern(30.0) ≈ 30 / expm1(30) rtol = 1.0e-14   # = 2.81e-12
    @test exprb_bern(30.0) < 1.0e-11                # z → +∞: B → 0, memory term dominates
end
...
@testitem "exprb_cap_exponent: the diagonal must not be annihilated" begin
    ...
    @test exprb_bern(710.0) == 0.0
    @test exprb_bern(89.0f0) == 0.0f0
    ...
    @test 0 < exprb_bern(exprb_cap_exponent(1.0e6)) < 1.0e-11
    b32 = exprb_bern.(exprb_cap_exponent.(Float32.(range(-100.0, 100.0, 501))))
    @test eltype(b32) === Float32 && all(isfinite, b32) && all(>(0), b32)
    @test exprb_bern(0.0f0) === 1.0f0
end

@testitem "exprb_bern: forming θ is what this avoids" begin
    using RAPID2D: exprb_bern
