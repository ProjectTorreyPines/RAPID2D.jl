# How snapshot storage is allocated, and when a snapshot is due.
#
# Two separate defects, both named in 201b1f8 as belonging in their own change:
#
#   1. Snapshot buffers were sized in the RAPID constructor from `t_end_s / snap*_Δt_s`
#      and allocated EAGERLY, with no reference to how many steps actually run. One
#      test config asked for 5001 2-D slots — 1.5 GiB — from a case taking 500 steps,
#      and never recorded one of them. Storage now grows with `push!` as snapshots are
#      taken, so an unrun interval costs nothing.
#
#   2. `snap*_Δt_s` was neither required to be ≥ `dt` nor an integer multiple of it.
#      `is_snap*_time` only ever fires within 0.1·dt of a multiple, so a non-integer
#      ratio silently DROPS snapshots, and `snap*_Δt_s` is also the DENOMINATOR of the
#      source/loss rates in `measure_snap0D!` — a mismatch corrupts the physics, not
#      just the sampling. Both are settled once in `validate_config!`.
...
@testitem "Snapshot storage is empty until a snapshot is taken" setup = [SnapAlloc] begin
    # The 201b1f8 case: t_end_s outruns the snapshot interval by 5000×. Sizing the
    # buffers from that ratio asked for 1.5 GiB before a single step had run. Nothing
    # may be allocated on the strength of `t_end_s` alone.
...
@testitem "The first snapshot records against no predecessor" setup = [SnapAlloc] begin
    # `measure_snap0D!` reads the PREVIOUS snapshot to form the growth rate. It indexed
    # `snaps0D[max(1, tid_0D - 1)]`, which on an empty vector is `snaps0D[1]` — a
    # BoundsError. Preallocation always handed it a zeroed slot, so the push! path that
    # already existed in `update_snaps0D!` had never actually been reached.
    RP = snap_case()
    @test isempty(RP.diagnostics.snaps0D)
...
@testitem "Snapshots accumulate in order without trailing blanks" setup = [SnapAlloc] begin
    # Preallocation left the unused tail zero-filled, and `snaps0D.time_s` (the
    # Vector getproperty overload) returned those zeros alongside real samples —
    # plot_snaps0D and check_energy_conservation consumed them as data.
    #
    # The run has to fall SHORT of config.t_end_s for the gap to show, which is the
    # 201b1f8 shape and an ordinary one: RAPID keeps its own `t_end_s` (types.jl), and
    # tests that chain run_simulation! calls move it. Sized from config.t_end_s = 1 µs
    # the buffers hold 51 and 21 slots; the run stops at 100 ns and fills 6 and 3.
    ...
    @test t0D[end] ≈ RP.t_end_s rtol = 1e-9
end

# ── Cadence guards ───────────────────────────────────────────────────────────────────

@testitem "A snapshot interval below dt is rejected" setup = [SnapAlloc] begin
    # Recording more often than the simulation steps is not a thing the solver can do.
    # It is also what asked for the 1.5 GiB: the interval divides t_end_s, so a
    # sub-dt interval inflates the count past the step count itself.
...
@testitem "A non-integer snapshot interval is snapped onto the dt grid" setup = [SnapAlloc] begin
    # 2.4·dt can never be hit: `is_snap*_time` is only ever evaluated at multiples of
    # dt, so the samples at 2.4, 7.2, 12.0 … fall between steps and vanish. Rounding to
    # 2·dt keeps every sample AND keeps snap0D_Δt_s honest as the rate denominator.
...
@testitem "An interval already on the dt grid passes silently" setup = [SnapAlloc] begin
    # A non-regression guard, not a new behaviour: the package defaults (dt = 10 µs,
    # snap0D = 20 µs, snap2D = 100 µs) and the convenience constructor's dt = 1 ns are
    # exact multiples, and warning on those would train everyone to ignore the warning
    # that matters.
    ...
    # The case that makes an exact `round(Δt/dt)*dt == Δt` test wrong. 9e-8 IS 3·dt at
    # dt = 3e-8, but the round-trip lands on 8.999999999999999e-8, so equality reports a
    # mismatch and would both warn and perturb a perfectly good interval. The interval
    # must come out untouched, to the bit — it is the rate denominator downstream.
...
@testitem "A snapped interval actually restores the dropped snapshots" setup = [SnapAlloc] begin
    # The end-to-end consequence of the rounding. At 2.4e-8 unsnapped, only the steps
    # landing within 0.1·dt of a 2.4e-8 multiple record at all — t = 0, 72, 96 ns out of
    # ten steps. Snapped to 2e-8 the series is complete: 0, 20, 40, 60, 80, 100 ns.
