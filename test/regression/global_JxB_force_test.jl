...
@testsnippet JxBForceAnalysis begin
    ...
    function analyze_force_balance_results(
            RP::RAPID;
        ...
        for i in eachindex(velocity_R_from_pos)
            ...
            if abs(measured_vel) + abs(computed_vel) > 1.0e-10
        ...
        if length(avg_aR_by_JxB) > 1 && length(avg_ueR) > 1
            for i in 2:min(length(avg_aR_by_JxB), length(avg_ueR))
                ...
                if abs(expected_vel) + abs(actual_vel) > 1.0e-10
        ...
        if length(avg_aR_by_JxB) > 5
            ...
            if length(early_indices) >= 3
                ...
                if k > 1.0e-10   # Linear acceleration model
                    # Solve: k*t³ + 3*a0*t² - 6*distance = 0 (Newton's method)
                    t_guess = sqrt(2 * distance_to_wall / max(mean(early_accels), 1.0e-10))

                    for iter in 1:10
                        f = k * t_guess^3 + 3 * a0 * t_guess^2 - 6 * distance_to_wall
                        df = 3 * k * t_guess^2 + 6 * a0 * t_guess
                        if abs(df) < 1.0e-15 || abs(f) < 1.0e-12
                            break
                        end
                        t_new = t_guess - f / df
                        if abs(t_new - t_guess) < 1.0e-8 || t_new <= 0
                            break
                        end
                        t_guess = t_new
                    end
        ...
        if verbose
            ...
            if wall_collision_time !== nothing
                println(@sprintf("  Wall collision at: %.1f μs", wall_collision_time * 1.0e6))
            end

            if expected_collision_time !== nothing
                println(@sprintf("  Expected collision time: %.1f μs", expected_collision_time * 1.0e6))
            end
        ...
        return (;
            times, ne_cen_R, ne_cen_Z, displacement_R, displacement_Z, total_ne,
    ...
    function create_force_balance_plots(
            RP, times, ne_cen_R, ne_cen_Z, total_ne, avg_aR_by_JxB, wall_collision_time;
            outdir::AbstractString = mktempdir(; cleanup = false)
        )
        # Convert times to ms for plotting
        times_ms = times * 1.0e3
        wall_collision_ms = wall_collision_time !== nothing ? wall_collision_time * 1.0e3 : nothing
        ...
        if length(ne_cen_R) > 1
            ...
            scatter!(
                p4, [ne_cen_R[1]], [velocity_R[1]],
                label = "Start", color = :green, markersize = 6
            )
            if length(velocity_R) > 1
                scatter!(
                    p4, [ne_cen_R[end - 1]], [velocity_R[end]],
...
@testitem "Global JxB Force" tags = [:regression] setup = [RegressionCommon, JxBForceAnalysis] begin
    ...
    config = regression_config(;
        ...
        dt = 2.5e-6,                    # [s]
        t_end_s = 150.0e-6,               # [s] long enough for the blob to reach the wall
        prefilled_gas_pressure = 1.0e-3,  # [Pa] (MATLAB: 1e-3)
        R0B0 = 3.0,                     # [T·m] vacuum toroidal field
        snap0D_Δt_s = 5.0e-6,             # [s] fine 0D cadence: the force-kinematics
        #     checks differentiate this time series
        snap2D_Δt_s = 20.0e-6,            # [s]
    )
    ...
    ini_n = gaussian_blob(RP.G; cenR, cenZ, radius, n0 = 1.0e16)  # [m⁻³]
    ...
    results = analyze_force_balance_results(
        RP;
    ...
    if verbose
        ...
        if results.wall_collision_time !== nothing
            println("  ✓ Wall collision at $(round(results.wall_collision_time * 1.0e6, digits = 1)) μs")
        end
        if !isempty(results.velocity_consistency_error)
